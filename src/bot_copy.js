...
console.log(process.env.CLIENT_ID1, process.env.CLIENT_ID2);

client.on('message', (message) => {
    // if(message.content === '!hello mama')
    //     console.log('working');
    if(message.author.id === process.env.CLIENT_ID2)
        return;
    if(message.author.id === process.env.CLIENT_ID1){
        if(!isNaN(message.content)){
            let num  = Number(message.content) + 1; 
...
//console.log(process.env.DJSTOKEN);
...
// //         msg.channel.send(`:ping_pong: pong! your ping is: *${bot.ping.toFixed()}ms*`);
// //     }
// //     if(cmd === 'purge'){
